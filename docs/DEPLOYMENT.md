...
gcloud run deploy cia \
  ...
  --set-env-vars "LLM_MODE=live,AUTH_MODE=live,GOOGLE_OAUTH_CLIENT_ID=306195530103-ub6t46i8sd9q1eatpt6dgo0i9811mnrp.apps.googleusercontent.com" \
  --min-instances 1 \
  --max-instances 8 \
  --cpu 1 \
  --memory 1Gi \
...
| `google token exchange failed: 401 … oauth2.googleapis.com/token` / Login says **401 invalid_client** / `/api/config` shows `"auth_credentials":"rejected"` | the client **ID** and client **secret** on the service are not a matching pair: the secret was regenerated in the console, the stored secret belongs to a *different* OAuth client (check you populated the secret the service actually reads — `GOOGLE_OAUTH_CLIENT_SECRET` → `cia-oauth-client-secret:latest`), or it was stored with a trailing newline (`echo` instead of `printf '%s'`) | Console → APIs & Services → Credentials → the web client whose ID is in `GOOGLE_OAUTH_CLIENT_ID` → copy **its** secret → add it as a new version of `cia-oauth-client-secret` exactly as in A7 (`printf '%s'`, no newline) → redeploy (the service reads `:latest` at start). `bash scripts/doctor.sh --check-only` prints `Google accepts the OAuth client id/secret pair` once fixed |
...
| GitHub **Deploy (Cloud Run · prod)** workflow fails at *Authenticate to Google Cloud (WIF)* | the Workload Identity pool/provider and `cia-deployer` SA from `terraform/envs/prod` do not exist yet (Terraform never applied), or the repository variables `GCP_PROJECT_ID` / `GCP_REGION` / `WIF_PROVIDER` / `DEPLOYER_SA` point elsewhere | the step after the failure prints the remedy: `terraform apply` in `terraform/envs/prod` (operator, gated), then set the four variables from `terraform output` (see `terraform/README.md`) and re-run. Until then deploy from Cloud Shell with `bash scripts/doctor.sh` |
